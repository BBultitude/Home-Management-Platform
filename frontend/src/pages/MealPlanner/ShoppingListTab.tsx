...
export default function ShoppingListTab() {
  ...
  const renderListContent = () => {
    if (loading) return <div className="text-center py-8 text-muted-foreground">Loading shopping list...</div>;
    if (!shoppingList || shoppingList.items.length === 0) return (
          ...
          <Table>
            ...
            <TableBody>
              {shoppingList.items.map((item: ShoppingListItem, index: number) => (
                <TableRow key={item.ingredient ?? index}>
                  <TableCell className="font-medium">{index + 1}</TableCell>
                  <TableCell>{item.ingredient}</TableCell>
                  <TableCell>{item.quantity}</TableCell>
                  <TableCell>
                    <div className="flex flex-wrap gap-1">
                      {item.recipe_names.map((recipeName: string) => (
                        <Badge key={recipeName} variant="outline" className="text-xs">
                          {recipeName}
                        </Badge>
                      ))}
                    </div>
